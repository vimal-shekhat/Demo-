...
class UsersController extends AppController {
	...
	/* User chatList with Userid Name Email and Profile */
		function chatList(){
			...
			if(!empty($user)){
				$query="SELECT h.id,p.user_id AS Userid, p.photo AS Photo, p.display_name AS display_name,u1.email AS Email FROM hearts h INNER JOIN profiles p ON p.user_id=h.fromid INNER JOIN USERS u1 ON u1.id = h.fromid INNER JOIN USERS u2 ON u2.id = h.toid and u2.id=$user[id] group by h.fromid";
				...
				if(!empty($results)){
					echo json_encode(array('result'=>array('status'=>'success','data'=>$results)));
					exit;
				}else{
					echo json_encode(array('result'=>array('status'=>'error','msg'=>'No any Hearts found.')));
					exit;
				}
